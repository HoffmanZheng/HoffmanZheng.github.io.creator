---
title: "Java：Collections、Arrays工具类及常见方法"
...
* HashMap 的key 的set 就是一个 HashSet
  HashSet 有的功能 HashMap 都有，所有 HashSet 里面包了一个 HashMap
...
* 是 `Google` 写的集合框架，在原生 JDK Collection 的基础上增添了更多有用的实现，如：`MultiSet` 可以存储相同的元素，并告诉你他们存了多少次；`BidirectionMap` 可以从 `value` 映射回 `Key` 值。 
...
* 当我们只需要存放元素值时，就选择实现Collection接口的集合，需要保证元素唯一时选择实现 Set 接口的集合比如 TreeSet 或 HashSet，不需要就选择实现List接口的比如ArrayList或LinkedList，然后再根据实现这些接口的集合的特点来选用。
