...
通过 [深入分析 Java I/O 的工作机制](https://chenghao.monster/2020/java-IO/) 我们知道 Java 中 InputStream 可以读取输入的字节流，而 `InputStreamReader` 则在 I/O 读取字节流的过程中完成了字节到字符的转换，这个解码过程它又委托 `StreamDecoder` 去做，这个过程中需要用户指定 Charset 编码格式。
